...
export const obtenerMatenimientoParaResumen = async (req, res) =>{
  ...
  JOIN
      tecnico T ON M.id_tecnico = T.id
  JOIN
      usuario U ON M.id_usuario = U.id_usuario WHERE  M.id_mantenimiento = ? ;
  ` 
...
export const registrarMantenimiento = async (req, res) => {
    try {
        ...
        console.log( transferencia, bienes)
              ...
              // Registra la asignación en la tabla 'asignacion'
               db.query(q, [transferencia.fecha_transferecia, transferencia.motivo, transferencia.costo, transferencia.usuario_id,  transferencia.id_tecnico], (err, data) => {
                console.log(err)
                if (err) return res.status(500).json(err);
    ...
    export const obtenerBienesMantenimiento = async(req, res) =>{
        ...
        console.log(id)
         try {
            const q = `SELECT ba.id_bien_mantenimiento, ba.id_bien, ba.id_mantenimiento, eb.id_variacion, eb.codigo_ejemplar, eb.ubicacion, eb.estado, eb.valor, b.nombre AS nombre_bien, b.descripcion AS descripcion_bien FROM bien_mantenimiento AS ba JOIN ejemplar_bien AS eb ON ba.id_bien = eb.id_variacion JOIN bien AS b ON eb.id_bien = b.id_bien WHERE ba.id_mantenimiento  = ?
        ` ;
     ...
     export const obtenerMantenimientoPorRangoDeFecha = async (req, res) => {
      ...
      JOIN
          tecnico T ON M.id_tecnico = T.id
      JOIN
          usuario U ON M.id_usuario = U.id_usuario WHERE  M.fecha_mantenimiento BETWEEN  ? AND ?` ;
      ...
      export const eliminarMantenimiento = async (req, res) => {
        ...
        try {
          ...
          db.query(verificarExistencia, [id], (err, data) => {
            ...
            const eliminarDepartamento = "DELETE FROM mantenimiento WHERE id_mantenimiento = ?";
            db.query(eliminarDepartamento, [id], (err, data) => {
              if (err) return res.status(500).json(err);
              return res.status(200).json("Mantenimiento eliminada correctamente");
            });
          });
        } catch (error) {
          console.error(error);
      ...
      export const finalizarMantenimiento = async (req, res) => {
        ...
        try {
            ...
            db.query(verificarExistencia, [id], (err, data) => {
                ...
                db.query(actualizarMantenimiento, [fechaHoy, id], (err, data) => {
                    if (err) return res.status(500).json(err);
                    ...
                    db.query(actualizarBienes, [id], (err, data) => {
                        if (err) return res.status(500).json(err);
